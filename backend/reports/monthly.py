...
def _categorize_entries(entries: list) -> dict[str, str]:
    ...
    prompt = (
        ...
        f"{section_list}\n\n"
        "Rules:\n"
        "- Each entry belongs to EXACTLY ONE section — the most appropriate one. Never repeat the same topic in multiple sections.\n"
        "- Use sentiment to decide the section:\n"
        "    • Clearly POSITIVE outcome (win, confirmation, award, above-target result) → what_was_good\n"
        "    • Clearly NEGATIVE or risky (delay, unresolved issue, forecast reduction, lost opportunity, operational risk) → less_satisfactory\n"
        "    • Market or competitor intelligence → market_competition\n"
        "    • Announcement, new partnership, upcoming event, structural change → news\n"
        "    • Neutral operational update (ongoing project, scheduled visit, process progress) → key_projects ONLY if it does not fit any of the above\n"
        "- Each distinct topic is a separate string in the JSON array\n"
